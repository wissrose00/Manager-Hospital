...
public class PrescriptionDetailFM extends Fragment {
    ...
    public View onCreateView(@NonNull LayoutInflater inflater, @Nullable ViewGroup container, @Nullable Bundle savedInstanceState) {
        View view = inflater.inflate(R.layout.prescitiption_detail, container, false);
        mapping(view);
        setup();
        init();
        setClick();
    ...
    private void  setClick(){
        ...
        btn2.setOnClickListener(v -> getConfirm());
    ...
    private void upDate(String sId){
        conn = new Connect();
        String sql= "UPDATE  Diagnosi SET status = 4 where id = '" + sId +"'";
        try {
            if (conn.exec(sql)) {
    ...
    private void filldata(String sId){
        ...
        try {
            ...
            ResultSet rs3 = conn.read("EXEC  [GetExaminateById10] '" + sId +"'"
            );
            ...
            if( rs1!= null) {
                while (rs1.next()) {
                    ...
                    tv4.setText(rs1.getString("doctor"));
                    tv5.setText(rs1.getString("department"));
                    tv6.setText(rs1.getString("patient"));
                    tv7.setText(rs1.getString("birth"));
                    tv8.setText(rs1.getString("gender"));
                    tv9.setText(rs1.getString("address"));
                    tv11.setText(rs1.getString("note"));
